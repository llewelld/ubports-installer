...
module.exports = async function(context) {
  ...
  if (context.targets.find(target => target.name === "deb")) {
    wrapperScript = `#!/bin/bash
      /opt/sailfish-installer/sailfish-installer.bin --no-sandbox "$@"
    `;
  } else if (context.targets.find(target => target.name === "appImage")) {
    wrapperScript = `#!/bin/bash
      "\${BASH_SOURCE%/*}"/sailfish-installer.bin --no-sandbox "$@"
    `;
  } else {
  ...
  fs.moveSync(
    path.join(distDir, "sailfish-installer"),
    path.join(distDir, "sailfish-installer.bin")
  );
  fs.writeFileSync(path.join(distDir, "sailfish-installer"), wrapperScript);
  fs.chmodSync(path.join(distDir, "sailfish-installer"), 0o765);
};
